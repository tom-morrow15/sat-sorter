...
export default {
  async fetch(request: Request, env: any): Promise<Response> {
    ...
    try {
      ...
      // Creates a Lightning invoice via Alby and stores it in D1 with the user's pubkey
      if (pathname === "/api/subscription/create-invoice" && request.method === "POST") {
        ...
        const albyResponse = await fetch(albyUrl.toString());
        const albyData = await albyResponse.json();

        if (!albyResponse.ok || !albyData.invoice?.pr) {
          console.error("Alby API error:", albyData);
          return json({ error: "Failed to create invoice" }, 500);
        }

        const bolt11 = albyData.invoice.pr as string;
        const verifyUrl = albyData.invoice.verify as string;
        const now = new Date();
        ...
        return json({
          invoice: albyData.invoice,
          invoiceId, // Frontend needs this to verify later
        });
      }
      ...
      if (pathname === "/api/subscription/verify-payment" && request.method === "POST") {
        ...
        // Call Alby's verify endpoint to check if the invoice was actually paid
        let albyVerifyResult: any;
        try {
          const verifyResponse = await fetch(invoice.verify_url as string);
          albyVerifyResult = await verifyResponse.json();
        } catch (err) {
          console.error("Failed to call Alby verify endpoint:", err);
          return json({ error: "Failed to verify payment with Alby" }, 502);
        }

        // Alby returns { settled: true } or { settled: false }
        const isPaid = albyVerifyResult?.settled === true;
